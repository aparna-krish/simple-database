...
//    int pageSize;
    ...
    */
    ...
    private boolean rollback(Long tid) throws IOException {
        ...
        try (RandomAccessFile raf = new RandomAccessFile(logFile, "r")) {
            ...
            Map<PageId, Page> rollbacks = new HashMap<>();
            ...
            for (Map.Entry<PageId, Page> entry : rollbacks.entrySet()) {
        ...
        */
        public void recover() throws IOException, IllegalStateException {
            synchronized (Database.getBufferPool()) {
                synchronized (this) {
                    recoveryUndecided = false;
                    // some code goes here
                    ...
                    if (cpLoc != -1) {
                        ...
                        // Read operation of the checkpoint record
                        int cpOper = raf.readInt();
                        raf.readLong();
                        // Check checkpoint record exists at location
                        if (cpOper != CHECKPOINT_RECORD) {
                            throw new RuntimeException("No checkpoint record here");
                        }
                        int numTransactions = raf.readInt();
                        // Restore tidss and their log record offsets from checkpoint
                        while (numTransactions-- > 0) {
                            long xid = raf.readLong();
                            long xoffset = raf.readLong();
                            tidToFirstLogRecord.put(xid, xoffset);
                        }
                        // Read and discard LSN
                        raf.readLong();
                    }

                    // Process each log record in the log file until the end of file is reached
                    while (true) {
                        try {
                            long currOffset = raf.getFilePointer();
                            int operation = raf.readInt();
                            long recordTid = raf.readLong();

                            if (operation == CHECKPOINT_RECORD) {
                                throw new RuntimeException("Already saw checkpoint, inconsistent log");
                            }
                            
                            else if (operation == BEGIN_RECORD) {
                                // Ensure begin is encountered only once per transaction
                                if (tidToFirstLogRecord.containsKey(recordTid)) {
                                    throw new RuntimeException("Already saw begin, inconsistent log");
                                }
                                tidToFirstLogRecord.put(recordTid, currOffset);

                            }
                            
                            else if (operation == ABORT_RECORD) {
                                // Rollback aborted transactions and remove from tidToFirstLogRecord
                                if (!tidToFirstLogRecord.containsKey(recordTid)) {
                                    throw new RuntimeException("Aborted, inconsistent log");
                                }
                            ...
                            else if (operation == COMMIT_RECORD) {
                                // Remove committed transactions from tidToFirstLogRecord
                                if (!tidToFirstLogRecord.containsKey(recordTid)) {
                                    throw new RuntimeException("Committed, inconsistent log");
                                }
                                tidToFirstLogRecord.remove(recordTid);
                            }
                            
                            else if (operation == UPDATE_RECORD) {
                                // Write updated page data to the database file
                                if (!tidToFirstLogRecord.containsKey(recordTid)) {
                                    throw new RuntimeException("Updated, inconsistent log");
                                }
                                Page before = readPageData(raf);
                                Page after = readPageData(raf);
                                ...
                                throw new IllegalStateException("Unexpected log record type");
                            }
                    ...
                    // Rollback lost transactions stored in tidToFirstLogRecord
